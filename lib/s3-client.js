...
const s3ClientExtensions = {
  ...
  isFileExists: async (bucketName, filePath) => {
    ...
    try {
      const metadata = await s3ClientExtensions.metadata(params.Bucket, params.Key)
  ...
  list: async (bucketName, folder) => {
    console.debug(`Scanning bucket ${bucketName}, prefix: ${folder}`)

    const allObjects = []
    for await (const s3Objects of s3ObjectsListGenerator(bucketName, folder)) {
      console.debug(`${folder}: added ${s3Objects.length} objects`)
   ...
   * Note: Caller is responsible for catching any errors! This simply awaits for the promise to resolve and returns s3ClientExtensions. Caller
  ...
  tag: async (bucketName, filePath, tagName) => {
    const tags = await s3ClientExtensions.tags(bucketName, filePath)
    if (!tags || !tags.TagSet) {
      return null
    }
  ...
  zipObjects: (bucketName, folder, objects) => {
    return s3Zip.archive({ s3: s3Client, bucket: bucketName, preserveFolderStructure: true }, folder, objects)
  },
  /**
   * The list of 'objects' to Zip can contain folders to. Simply append those with '/' and this method
   * will regard them as folders. Then the files underneath those folders will added to the list of the
  ...
  zipObjectsToBucket: (fromBucketName, fromFolder, objects, toBucketName, toPath) => {
    const { writeStream, uploadPromise } = s3ClientExtensions.createWriteStream({
      Bucket: toBucketName,
      Key: toPath
    })

    /*
     * We do it this way to ensure s3ClientExtensions both the pipeline AND the upload Zip to S3 promise are
    ...
    return new Promise((resolve, reject) => {
      expandFolders(fromBucketName, objects).then(finalObjectList => {
        pipeline(s3ClientExtensions.zipObjects(fromBucketName, fromFolder, finalObjectList), writeStream).then(() => {
  ...
  updateClient: (opts) => {
    ...
    Object.assign(s3Client, s3ClientExtensions)
  }
}
...
/**
 * Async generator-style function s3ClientExtensions allows the caller to use 'for await' construct to iterate over
 * results s3ClientExtensions may require multiple asynchronous calls to S3's listObjectsV2() api
 * */
async function* s3ObjectsListGenerator (bucketName, folder) {
  const s3 = s3Client
  const opts = {
    Bucket: bucketName,
    Prefix: folder
 ...
 * contained in it.
 */
async function expandFolders(bucket, objects) {
  const retObjects = []
  for (const obj of objects) {
    ...
    for (const item of await s3ClientExtensions.list(bucket, obj)) {
      retObjects.push(`${obj}${item.Key}`)
    }
  }
  return retObjects
}
