...
const s3ClientWrapper = {
  ...
  listAsStream: (bucketName, folder) => {
    let buffer = []
    ...
    return new Readable({
      ...
      async read() {
        ...
        const items = buffer
        buffer = []
        const res = await s3ObjectsListGenerator(bucketName, folder, continuationToken).next()
  ...
  sync: ({
    ...
    return pipeline(
      fromS3Client.listAsStream(fromBucket, fromPath),
      new Writable({
        objectMode: true,
        async write(chunk, encoding, callback) {
          try {
            ...
            s3ClientWrapper.createReadStream({ Bucket: fromBucket, Key: chunk.Key }).pipe(pass)
          } catch (e) {
 ...
 * */
async function * s3ObjectsListGenerator(bucketName, folder, continuationToken = undefined) {
  const s3 = s3Client
  const opts = {
    Bucket: bucketName,
    Prefix: folder,
