...
const extensions = {
  ...
  isFileExists: async (bucketName, filePath) => {
    ...
    try {
      const metadata = await extensions.metadata(params.Bucket, params.Key)
   ...
   * Note: Caller is responsible for catching any errors! This simply awaits for the promise to resolve and returns extensions. Caller
  ...
  tag: async (bucketName, filePath, tagName) => {
    const tags = await extensions.tags(bucketName, filePath)
    if (!tags || !tags.TagSet) {
      return null
    }
  ...
  zipObjectsToBucket: (fromBucketName, fromFolder, objects, toBucketName, toPath) => {
    const { writeStream, uploadPromise } = extensions.createWriteStream({
      Bucket: toBucketName,
      Key: toPath
    })

    /*
     * We do it this way to ensure extensions both the pipeline AND the upload Zip to S3 promise are
    ...
    return new Promise((resolve, reject) => {
      pipeline(extensions.zipObjects(fromBucketName, fromFolder, objects), writeStream).then(() => {
...
Object.assign(s3Client, extensions)
module.exports = s3Client

/**
 * Async generator-style function extensions allows the caller to use 'for await' construct to iterate over
 * results extensions may require multiple asynchronous calls to S3's listObjectsV2() api
 * */
