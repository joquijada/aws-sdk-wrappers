...
const s3ClientExtensions = {
  ...
  list: async (bucketName, folder) => {
    ...
    console.debug(`Retrieved ${allObjects.length} s3 objects: ${JSON.stringify(allObjects)}`)
    return allObjects
  },
  /**
...
async function expandFolders(bucket, folder, objects) {
  ...
  for (const obj of objects) {
    ...
    for (const item of await s3ClientExtensions.list(bucket, `${folder}/${obj}`)) {
      // Make the paths relative to 'folder'. AWS S3 gives back paths which are absolute
      retObjects.push(item.Key.replace(`${folder}/`, ''))
    }
  }
  return retObjects
}
