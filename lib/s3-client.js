...
const s3ClientWrapper = {
  ...
  listAsStream: ({ bucketName, folder, startAfter = undefined }) => {
    let buffer = []
    let continuationToken = '__START__'
    return new Readable({
      objectMode: true,
      async read() {
         ...
         * buffer became full, so we have to keep going until we process all items in our buffer, all the while skipping
         * calls to the S3 list API (to avoid looping infinitely over the S3 objects).
         */
        if (continuationToken) {
          if (continuationToken === '__START__') {
            // The __START__ token is just to let us fetch the first set of results
            // from S3
          ...
          continuationToken = res.value.continuationToken // save for next call to this._read()
          res.value && res.value.contents && items.push(...res.value.contents)
        }
        ...
        if (items.length < 1) {
          ...
          // Push each item onto stream's internal buffer to pass it to downstream pipe
          let isFull = false
          for (const item of items) {
            if (!isFull) {
              isFull = !this.push(item)
            } else {
              // Internal buffer full? Save to our own local "buffer" for next call to this._read()
