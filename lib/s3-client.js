...
const s3ClientWrapper = {
  ...
  zipObjectsToBucket: (fromBucketName, fromFolder, objects, toBucketName, toPath) => {
    const { writeStream, uploadPromise } = s3ClientWrapper.createWriteStream({
      Bucket: toBucketName,
      Key: toPath
    })
...
/**
 * Async generator-style function s3ClientWrapper allows the caller to use 'for await' construct to iterate over
 * results s3ClientWrapper may require multiple asynchronous calls to S3's listObjectsV2() api
 * */
...
async function expandFolders(bucket, folder, objects) {
  ...
  for (const obj of objects) {
    ...
    for (const item of await s3ClientWrapper.list(bucket, `${folder}/${obj}`)) {
